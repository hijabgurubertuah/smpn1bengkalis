...
export const RichTextEditorWithImages: React.FC<RichTextEditorWithImagesProps> = ({
  ...
  // Highlight selected embed node directly with a prominent blue ring
  useEffect(() => {
    if (selectedEmbedNode) {
      selectedEmbedNode.classList.add('ring-4', 'ring-blue-600', 'ring-offset-2', 'rounded-xl', 'shadow-md', 'transition-all');
      return () => {
        selectedEmbedNode.classList.remove('ring-4', 'ring-blue-600', 'ring-offset-2', 'rounded-xl', 'shadow-md', 'transition-all');
      };
    }
  }, [selectedEmbedNode]);
  ...
  return (
    <div className="space-y-2">
      ...
      <div className={`border border-slate-300 rounded-xl overflow-hidden bg-white shadow-2xs transition-all relative ${!selectedEmbedNode ? 'focus-within:ring-2 focus-within:ring-blue-600 focus-within:border-blue-600' : ''}`}>
          ...
          {/* Floating Minimalist Action Overlay for Selected Image or Video Embed */}
          {selectedEmbedNode && (
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-50 bg-slate-900/95 backdrop-blur-md text-white rounded-xl p-1.5 flex items-center justify-center gap-1.5 shadow-2xl border border-slate-700/80 animate-in fade-in slide-in-from-bottom-2 duration-200">
              {/* T Panah Atas (Teks di atas) */}
              <button
                type="button"
                onClick={handleInsertParagraphBefore}
                className="p-1.5 text-xs font-bold text-emerald-400 hover:text-emerald-300 hover:bg-emerald-950/80 active:scale-95 rounded-lg border border-emerald-700/50 flex items-center gap-0.5 cursor-pointer shrink-0"
                title="Teks di Atas"
              >
              ...
              {/* Ganti Gambar (Image Icon) */}
              <button
                type="button"
                onClick={handleEditSelectedEmbed}
                className="p-1.5 text-xs font-bold text-blue-400 hover:text-blue-300 hover:bg-blue-950/80 active:scale-95 rounded-lg border border-blue-700/50 flex items-center justify-center cursor-pointer shrink-0"
                title="Ganti Foto / Layout"
              >
                <ImageIcon className="w-4 h-4" />
              </button>
              ...
              <button
                ...
                className="p-1.5 text-xs font-bold text-red-400 hover:text-red-300 hover:bg-red-950/80 active:scale-95 rounded-lg border border-red-700/50 flex items-center justify-center cursor-pointer shrink-0"
                title="Hapus Media"
              >
              ...
              <button
                ...
                className="text-slate-400 hover:text-white transition-colors cursor-pointer p-1 rounded-lg hover:bg-slate-800 shrink-0"
                title="Batal Pilih"
              >
                <X className="w-4 h-4" />
              </button>

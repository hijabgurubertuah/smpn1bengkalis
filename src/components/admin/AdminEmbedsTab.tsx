...
import { Video, MapPin, Search, RotateCcw, CheckCircle2 } from 'lucide-react';
import {
...
export const AdminEmbedsTab: React.FC<AdminEmbedsTabProps> = ({ config, onChange }) => {
  ...
  const handlePerformSearch = (queryOverride?: string) => {
    ...
    // Use intelligent converter to support iframe tags, URLs, or queries
    const result = convertToGoogleMapsEmbedUrl(q, 'SMP Negeri 1 Bengkalis, Bengkalis Kota');
    const newUrl = result.embedUrl || buildGoogleMapsEmbedUrl(q, 17);
  ...
  const handleSetOfficialMap = () => {
    setSearchLocation('SMPN 1 Bengkalis');
    setPreviewMapUrl(OFFICIAL_SMPN1_MAP_EMBED_URL);
    onChange({
      ...config,
      embeds: {
        ...embeds,
        mapTitle: 'Lokasi SMP Negeri 1 Bengkalis',
        mapIframeUrl: OFFICIAL_SMPN1_MAP_EMBED_URL,
      },
    });
  };

  const handleReset = () => {
    handleSetOfficialMap();
  };
      ...
      <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-xs space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={handleSetOfficialMap}
              className="px-2.5 py-1 text-xs font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-md border border-emerald-200 flex items-center gap-1 cursor-pointer transition-colors"
              title="Gunakan embed peta resmi SMPN 1 Bengkalis"
            >
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
              <span>Peta Resmi SMPN 1</span>
            </button>
            <button
              type="button"
              onClick={handleReset}
